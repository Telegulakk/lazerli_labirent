...
uint8_t rx_buffer; // UART üzerinden asenkron olarak alınacak 1 Byte'lık komutu tutan değişken
...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
volatile int servo_1_pwm = 1500;
volatile int servo_2_pwm = 1500;
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
	...
	//servolar istenen pwm degerine getirildi
	__HAL_TIM_SET_COMPARE(&htim3, TIM_CHANNEL_1, servo_1_pwm);
	__HAL_TIM_SET_COMPARE(&htim3, TIM_CHANNEL_2, servo_2_pwm);

	// --- 2. UART Kesmesinin (Interrupt) Başlatılması ---
	// USART2 üzerinden 1 byte veri geldiğinde donanım kesmesi oluşturmasını sağlar.
...
static void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(LAZER_SIGNAL_GPIO_Port, LAZER_SIGNAL_Pin, GPIO_PIN_RESET);

  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(BUZZER_GPIO_Port, BUZZER_Pin, GPIO_PIN_RESET);

  /*Configure GPIO pin : LAZER_SIGNAL_Pin */
  GPIO_InitStruct.Pin = LAZER_SIGNAL_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
  HAL_GPIO_Init(LAZER_SIGNAL_GPIO_Port, &GPIO_InitStruct);
  ...
  /*Configure GPIO pins : LDR1_Pin LDR2_Pin */
  GPIO_InitStruct.Pin = LDR1_Pin|LDR2_Pin;
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart) {
	// Gelen kesmenin bizim beklediğimiz USART2 hattından gelip gelmediği kontrol edilir
...
/*
 ...
 {
 if (servo_1_pwm < 2000) servo_1_pwm += 20;
 }
 else
 {
 if (servo_1_pwm > 1000) servo_1_pwm -= 20;
 }
 __HAL_TIM_SET_COMPARE(&htim3, TIM_CHANNEL_1, servo_1_pwm);
 }
 }
